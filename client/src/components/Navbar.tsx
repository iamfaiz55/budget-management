...
const Navbar = ({ isOpen, setIsOpen }: { isOpen: boolean; setIsOpen: (open: boolean) => void }) => {
  ...
  return (
    <nav
      className={`hidden md:flex fixed top-0 left-0 h-screen bg-white shadow-lg border-r border-gray-200 p-5 flex-col transition-all duration-300 ${
        isOpen ? "w-60" : "w-24"
      }`}
    >
      {/* Toggle Button */}
      ...
      <div className="mt-12 flex-1 overflow-y-auto no-scrollbar">
        ...
        {[
          ...
          { id: "navTotal", name: "Total", path: "/user/total", icon: <FaMountain /> },
          { id: "navStatistics", name: "Statistics", path: "/user/stats", icon: <FaChartPie /> },
          { id: "navAccount", name: "Account", path: "/user/account", icon: <FaUser /> },
          { id: "navFamily", name: "Family", path: "/user/family", icon: <IoIosPeople size={30} /> },
          { id: "navMore", name: "More", path: "/user/more", icon: <TiThMenu /> },
        ].map((link) => (
          <NavLink
            key={link.name}
            to={link.path}
      ...
      >
        ...
        {isOpen && <span onClick={()=> logout()} className="text-md font-medium">Logout</span>}
      </button>
    </nav>
  );
};
